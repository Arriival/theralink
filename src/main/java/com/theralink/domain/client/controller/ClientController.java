...
public class ClientController extends BaseController {
	...
	public ClientViewModel load(@PathVariable String id) {
		return ModelMapperUtil.map(iCustomerService.authorizedLoad(id), ClientViewModel.class);
	}

	@GetMapping(value = "/file/{id}")
	public CustomerFileInfoViewModel file(@PathVariable String id) {
		return ModelMapperUtil.map(iCustomerService.authorizedLoad(id), CustomerFileInfoViewModel.class);
	}
	...
	public boolean delete(@PathVariable String id) {
		return iCustomerService.deleteByAuthorizedId(id);
	}
