package com.theralink.repository.references;
...
public interface IReferencesRepository extends IGenericRepository<References, String> {
	...
	@Query("SELECT e FROM References e WHERE e.referrer.person.id = :referredFromPersonId and (:customerId is null or e.client.id = :customerId) order by e.createdDate desc ")
